...
export default function Modal({ title, children, onClose, size = 'md' }: ModalProps) {
  ...
  return (
    <div
      ...
      aria-label={title}
    >
      <div className={`bg-white rounded-[2rem] shadow-2xl w-full ${widthClass} animate-fade-in flex flex-col max-h-[90vh] overflow-hidden border border-slate-100`}>
        <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 shrink-0 bg-slate-50">
          <h2 className="font-display font-black text-brand-green-900 text-xl uppercase tracking-wider">
            {title}
          </h2>
          <button
            onClick={onClose}
